...
public class OCR {
  ...
  /**
   * 接口说明： https://github.com/AvensLab/OcrKing/blob/master/线上识别http接口说明.txt
   */
  ...
  public static String ocrCode(String imgFilePath, int limitCodeLen) {
    ...
    try {
      html = HttpClientUtil
          .upload(HttpConfig.custom().client(client).url(apiUrl).files(new String[]{imgFilePath}, "ocrfile", true).map(getParaMap()));
